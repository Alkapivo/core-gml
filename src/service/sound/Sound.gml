...
function Sound(_asset, config = {}) constructor {
  ...
  pause = function() {
    if (this.isPlaying()) {
      this.cachedTimestamp = audio_sound_get_track_position(this.soundId)
  ...
  getPosition = function() {
    if (this.isPlaying()) {
      var position = audio_sound_get_track_position(this.soundId)
      if (Optional.is(this.cachedTimestamp)) {
        position = this.cachedTimestamp
        this.cachedTimestamp = null
      }
      return position
    } else {
